...
**Watch for:** Updating every column every row; tracing what the code *should* do rather than what it says; skipping the final loop check. **Trace-table convention (binding here and in any later module):** one row = one line = one variable assignment. Never merge two different lines' assignments into a single row, even when they read naturally as "the same step" (a loop counter's `FOR` line and its body line; an `IF`-guarded assignment and the line that set the value being tested). A loop counter's value change belongs on the row for the `FOR` statement's own line — never the line after it. Any other line that changes a variable — loop body, `THEN` branch, anything — gets its own separate row, pointed at that line specifically, and only appears when that line actually executes.
