...
| W1 | The Blueprint Shelf (additive blueprints) | W | cross-topic |
| W2 | The Blueprint Shelf II (multiplicative blueprints) | W | cross-topic |
| W3 | Blueprint Chains (multi-step & mixed) | W | cross-topic |
...
### The W routine — seven verbs (the owner's strategy, refined together; this exact card appears in every W module and travels into the Bridge)

Blueprints themselves are governed by `blueprint-architecture.md` — minted by him, never issued, coached by stress-testing. The card:

1. **READ** — underline each word as you read it. Circle every number and its unit. An unmarked word has not been read yet — go back for it. (Or play the question aloud and follow along.)
2. **FIND** — draw a box around the part that tells you what to find. Test: "We need to find…" in front of it makes sense. (It usually ends with a question mark, or starts with *find*, *work out*, or *calculate*.) Then write what it's asking in your own words next to it, read both, and check they ask for the same thing.
3. **LIST** — go through the question again. Try "We know that…" in front of each part. If it makes sense, it's a fact: copy it to your list and tick its words in the question. Word facts count exactly like number facts.
4. **MATCH** — pick the blueprint from your library whose gaps your facts can fill, and lay it out with its gaps showing.
5. **FILL** — write your facts into the gaps in the blueprint or formula. A fact with no gap is not needed. The gap left empty is where your answer will go.
6. **WORK** — every blueprint and formula has its rule written on it as a sentence, like "total = number of groups × size of each". Write your numbers into that sentence and work out the empty gap.
7. **CHECK** — two checks. **Question check:** read your boxed question from step 2 again — is your answer the exact thing it asked you to find? **Number check:** write your answer into its empty gap, so every gap is now full; read the rule with your numbers in it — "3 groups of 25 kg makes 75 kg" — and check the arithmetic is true.

Design constraints (load-bearing, from the owner's editing passes — preserve exactly): every step is a physically checkable action (underline, circle, box, tick, prefix-test); no term is used before it is defined; importance is never judged while reading — relevance is decided mechanically at FILL; the two prefix tests ("We need to find…", "We know that…") are the only fact/ask classifiers; every blueprint and formula displays its rule as a written sentence on its face; the four marks (underline = read, circle = number+unit, box = the ask, tick = fact copied) never collide and port directly to pencil-and-paper exam habit.

**W1 · The Blueprint Shelf** — cross-topic (worded problems; minting THE SUM) | N3
Hook: "Worded questions feel like riddles because school teaches trigger-word hunting ('altogether means add'). That trick fails — and there's a method that doesn't. Six of the seven verbs you can already do; today you mint your first blueprint." Doors: SEE the routine run on a worked example, all four marks appearing; HOLD the routine physically — tiles into gaps, numbers veiled until FILL; COUNT nothing yet — structure before arithmetic, always. Signature: **THE SUM is minted** (per the architecture: hollowed from his own solved example — whole = one part + the other part) and immediately meets its three disguises: putting-together, before-and-after, more-than/less-than. The myth-busting exhibit: a question containing "more" that needs subtraction, watched breaking the trigger-word method (per A12) — which doubles as THE SUM's mandatory stress test. Watch for: residual trigger-word habits; grabbing numbers before structure (the veiling exists for this); tells treated as the test (the tell is a hint; whether the gaps take the facts is the test — say so).

**W2 · The Blueprint Shelf II** — cross-topic (minting THE PRODUCT) | W1, N10
Hook: "Your second great blueprint: anything made of same-sized pieces. With these two, every one-step worded question in the subject is yours." Doors: SEE THE PRODUCT hollowed from a solved bundles example (total = how many × how big of each); HOLD its three disguises — bundles, per-one (*per*, *each*), resizing — interleaved with W1 questions (mixed practice is where MATCH becomes real); COUNT via the ratio table where the disguise calls for it. Signature: **THE PRODUCT minted and stress-tested** — the mandatory test: a question giving the total and asking for a part (division wearing multiplication's clothes; his blueprint must survive being read backwards, which WORK's rule-sentence makes natural). The multiplicative-thread motif marks the resizing disguise. Watch for: SUM forced onto PRODUCT questions (the gaps refuse the pieces — let them); per-one and resizing confused (the gap names arbitrate).

**W3 · Blueprint Chains** — cross-topic (multi-step & mixed; minting process blueprints) | W1–W2, most strands
Hook: "Long questions aren't harder maths — they're two or three blueprints in a row, one's filled gap becoming the next one's fact." Doors: SEE long questions decomposed into chained gap-diagrams (a visible pipeline); HOLD building the chain from his library; COUNT each link with tools he owns, CHECK running at every link and once more at the end against the boxed question. Signature: **the chain bench** — multi-step questions as blueprint pipelines, footprints flowing between links (this IS method-marks writing, never named pre-Bridge); the common 0580 process patterns (percentage change and reverse, best-buy, ratio sharing, compound shapes as split-then-SUM) are minted here as process blueprints per the architecture's catalogue. Contexts deliberately varied per A10 — the same chains in money, gardens, journeys, recipes — making W3 the course's generalisation engine and the on-ramp to Bridge stage P1. Watch for: correct chain, links worked in the wrong order (the pipeline shows the dependency); intermediate answers rounded early (N11 callback — fog travels down pipelines and grows).
