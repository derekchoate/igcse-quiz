  ...
  function makeByte(rowEl,labeled,onChange){
    ...
    const api={
      ...
      set(n){api.setBitsArr(W.map(v=>(n&v)?1:0));},
      shiftLeft(){
        const old=api.bitsArr();
        const lost=old[0]===1;
        const next=old.slice(1);next.push(0);
        api.setBitsArr(next);
        return lost;
      },
      shiftRight(){
        const old=api.bitsArr();
        const lost=old[7]===1;
        const next=old.slice(0,7);next.unshift(0);
        api.setBitsArr(next);
        return lost;
      },
      flip(){api.setBitsArr(api.bitsArr().map(b=>1-b));},
      addOne(){
        const next=api.bitsArr();
        let carry=1;
        for(let i=7;i>=0&&carry;i--){
          const s=next[i]+carry;
          next[i]=s%2;carry=s>=2?1:0;
        }
        api.setBitsArr(next);
      }
    };
